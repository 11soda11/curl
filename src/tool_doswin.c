...
static int swrite_blocking(curl_socket_t sock,
                           unsigned char* data,
                           size_t len) {
  fd_set fdwrite;
  fd_set fdexcep;
  ...
  do {
    ...
    if(FD_ISSET(sock, &fdexcep)) {
      int sock_err = 0;
      int sock_err_size = sizeof(sock_err);
      getsockopt(sock, SOL_SOCKET, SO_ERROR, (char *)&sock_err, &sock_err_size);
      errorf("connect failure: %d", sock_err);
      return -1;
    }

    ret = swrite(sock, data + nwritten,
                 len - nwritten);
...
static int verify_peer(curl_socket_t nonblock_sock_to_verify,
                       curl_socket_t verifying_sock) {
  curl_socket_t send_sk = nonblock_sock_to_verify;
  curl_socket_t recv_sk = verifying_sock;
  uint64_t auth_correct;
  uint64_t maybe_auth_correct;
  size_t nread = 0;

  if(curlx_win32_random((unsigned char *)&auth_correct, sizeof(auth_correct))) {
    errorf("curlx_win32_random() error");
    return -1;
  }

  /* write the auth val */
  if(swrite_blocking(send_sk, (unsigned char *)&auth_correct, sizeof(auth_correct)) == -1) {
    return -1;
  }


  /* verify the auth val on the other socket */
  do {
    ssize_t ret = sread(recv_sk, ((unsigned char *)&maybe_auth_correct) + nread,
                        sizeof(maybe_auth_correct) - nread);
    if(ret <= 0) {
      if(!ret) {
    ...
    nread += ret;
  } while(nread < sizeof(maybe_auth_correct));

  if(maybe_auth_correct != auth_correct) {
...
curl_socket_t win32_stdin_read_thread(void)
{
  ...
  curl_socket_t socket_w = CURL_SOCKET_BAD;
  ...
  do {
    ...
    /* make the receiving socket nonblocking */
    ...
    socket_w = CURL_ACCEPT(socket_l, NULL, NULL);
    ...
    tdata->socket_w = socket_w;

    if(verify_peer(socket_r, socket_w))
      break;
